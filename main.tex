...
\begin{frame}
  ...
  \begin{itemize}
    \item Dada una curva cualquiera, existe un único representante proyectivo y liso en su clase de equivalencia birracional.
      \pause
    \item Algebraicamente:~es la curva abstracta formada por los anillos de valuación discreta de su cuerpo de fracciones.
      \pause
    \item Geométricamente podemos obtenerla como la normalización de una compacitifación de la curva.
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Problema:~blow-up produce nuevo representante distinto pero también proyectivo y liso.
      \pause
    \item Solución:~criterio de Castelnuovo nos permite detectar divisores excepcionales.
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item $\mathbf{D}^{\mathrm{b}}(X) = \langle \mathbf{A}, \mathbf{B} \rangle$ significa que no hay morfismos no nulos de $\mathbf{B}$ a $\mathbf{A}$ y que la subcategoría triangulada más pequeña de $\mathbf{D}^{\mathrm{b}}(X)$ que contiene a $\mathbf{A}$ y a $\mathbf{B}$ es $\mathbf{D}^{\mathrm{b}}(X)$.
      \pause
    \item El primer ejemplo se debe a Be\u{\i}linson:
      \[ \mathbf{D}^{\mathrm{b}}(\mathbf{P}^{n}) = \langle \mathscr{O}, \mathscr{O}(1), \ldots, \mathscr{O}(n) \rangle. \]
  \end{itemize}
\end{frame}
